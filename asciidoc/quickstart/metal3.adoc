...
----
+
. Note that it can take around two minutes for the initContainer to run on this deployment, so ensure the pods are all running before proceeding:
+
[,shell]
----
...
Install https://cluster-api.sigs.k8s.io/user/quick-start.html#install-clusterctl[clusterctl] 1.6.x, after which we will install the core, infrastructure, bootstrap and controlplane providers as follows:
...
For test/PoC environments where the management cluster is a single node, it is possible to avoid the requirement for an additional IP.

In this mode, the endpoint for the management cluster APIs is the IP of the management cluster, thus it should be reserved when using DHCP

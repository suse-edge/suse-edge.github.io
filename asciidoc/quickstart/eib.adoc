...
Configuring the operating system is optional and should not be included unless one or more customizations are being applied.
...
image:
  imageType: iso
  arch: x86_64
  baseImage:  SL-Micro.x86_64-6.2-Base-RT-SelfInstall-Kiwi.iso
...
The above example generates an ISO image which is used to trigger a `libvirt` domain with `virt-install`:
...
virt-install \
      ...
      --hvm \
      --connect qemu:///system \
      --description "Server#1 with EIB SLMicro 6.1 RT" \
      --osinfo slem6.1 \
      --boot uefi,hd,cdrom,network,bootmenu.enable="yes",bootmenu.timeout="60" \
      ...
      --memory 16204 \
      --vcpus 4 \
      --disk ./eib/eib-Base-RT-SelfInstall.iso,device=cdrom,target.bus=sata \
      --disk size=60,format=qcow2,target.bus=sata,serial=111-disk1 \
      --disk size=60,format=qcow2,target.bus=sata,serial=222-disk2 \
      --network network=default,model.type='virtio',mac.address="52:54:21:2e:1e:20" \
      --graphics=vnc \
      --console pty \
      --serial pty \
      --name server1-eib
...
[NOTE]
====
The `serial` attribute value for the first defined SATA disk (`serial=111-disk1`) matches the `installDevice` value (`/dev/disk/by-id/ata-QEMU_HARDDISK_111-disk1`).`libvirt` automatically adds the `ata-QEMU_HARDDISK_prefix` to the final ID of the defined ATA devices. 
====
...
* `isoConfiguration` - Optional; configuration in this section only applies to ISO images.
  * `installDevice` - Optional; specifies the disk that should be used as the install device. This needs to be block special, and will default to automatically wipe any data found on the disk.

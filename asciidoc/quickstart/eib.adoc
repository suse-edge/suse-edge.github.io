...
[NOTE]
====
The `serial` attribute value for the first defined SATA disk (`serial=111-disk1`) matches the `installDevice` value (`/dev/disk/by-id/ata-QEMU_HARDDISK_111-disk1`).`libvirt` automatically adds the `ata-QEMU_HARDDISK_` to the final ID of the defined ATA devices. 
====

...
In the case of *Day 2* operations, *SUC* is used to interpret the following *Plan* resources:

. `OS package update Plan` - instructs *SUC* how to perform an update of the packages of a specific cluster node. For information on how to deploy this *Plan*, refer to <<day2-os-package-update>>.

. `Kubernetes version upgrade Plan` - instructs *SUC* how to perform an upgrade of the Kubernetes distribution that a specific cluster node is running. For information on how to deploy this *Plan*, refer to <<day2-k8s-upgrade>>.

[NOTE]
====
This section focuses solely on deploying the `system-ugprade-controller`. *Plan* resources should be deployed from the previously mentioned documentations.
====
...
Deployment orchestration for *SUC* is done by using either a <<day2-suc-dep-gitrepo,GitRepo>> or <<day2-suc-dep-bundle,Bundle>> resource.
...
`GitRepo` creation can be done either though the <<day2-suc-dep-gitrepo-rancher,Rancher UI>> (when `Rancher` is available) or by <<day2-suc-dep-gitrepo-manual, manual deployment>>. For additional `GitRepo` information, see the link:https://fleet.rancher.io/ref-gitrepo[GitRepo Resource] documentation.

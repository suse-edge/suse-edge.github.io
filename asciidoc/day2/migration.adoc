...
This section covers how to migrate existing `Edge 3.0` <<day2-migration-mgmt,management>> and <<day2-migration-downstream, downstream>> clusters to `Edge 3.1`.
...
This section covers how to migrate a `management` cluster from `Edge 3.0` to `Edge 3.1`.
...
. <<day2-migration-mgmt-helm, SUSE Helm charts>>


[#day2-migration-mgmt-os]
=== Operating System (OS)

This section covers the steps needed to migrate your `management` cluster nodes' OS to an `Edge 3.1` supported version.
...
* `Mirror SUSE RPM repositories` - RPM repositories related to the operating system that is specified in the `Edge 3.1` <<release-notes,release>> should be locally mirrored, so that `transactional-update` has access to them. This can be achieved by using either link:https://github.com/SUSE/rmt[RMT] or link:https://documentation.suse.com/suma/4.3/en/suse-manager/index.html[SUMA].
...
. After a successful `transactional-update` run, for the changes to take effect on the system you would need to execute a `reboot`:
+
[,bash]
----
reboot
...
You can upgrade the RKE2 version to a `Edge 3.1` compatible version using the RKE2 installation script as follows:
...
. Use the RKE2 installation script to install the correct `Edge 3.1` compatible RKE2 version:
+
[,bash]
----
...
* How to <<day2-migration-mgmt-helm-eib, upgrade SUSE Helm charts>> deployed through <<components-eib,EIB>>.

* How to <<day2-migration-mgmt-helm-non-eib, upgrade SUSE Helm charts>> deployed through non-EIB means.
...
With the current RKE2 version that `Edge 3.1` utilizes, there is an issue where all ingresses that do not contain an `IngressClass` are ignored by the ingress controller. To mitigate this, users would need to manually add the name of the default `IngressClass` to the default `Rancher` Ingress. 
...
* If `Rancher` was deployed through Helm, add the `--set ingress.ingressClassName=nginx` flag to your upgrade command:
+
[,bash]
----
helm upgrade rancher <chart_repo>/rancher --version <version> --set ingress.ingressClassName=nginx --values <chart_name>-values.yaml -n <chart_namespace>
----
+
For a full example of how to utilize this command, see the following <<day2-migration-mgmt-helm-non-eib-example, example>>.

[#day2-migration-mgmt-helm-capi]
==== Cluster API controllers migration

From `Edge 3.1`, Cluster API (CAPI) controllers on a Metal^3^ management cluster are managed via the `Rancher Turtles` Helm chart.

To migrate the CAPI controllers versions to `Edge 3.1` compatible versions, install the `Rancher Turtles` chart:
...
After some time, the controller pods running in the `capi-system`, `capm3-system`, `rke2-bootstrap-system` and `rke2-control-plane-system` namespaces will be upgraded with the `Edge 3.1` compatible controller versions.
...
==== SUSE Helm chart upgrade - EIB

This section covers how to upgrade a SUSE Helm chart that has been deployed through <<components-eib, EIB>> on an `Edge 3.0` release.
...
This means that, before upgrading to an `Edge 3.1` compatible version, any Helm charts deployed on an `Edge 3.0` environment using EIB should have their chart manifests removed from the manifests directory of the relevant Kubernetes distribution.
...
** For charts hosted on a Helm chart repository:
+
[,bash]
----
...
This section provides an example of upgrading the `Rancher` and `Metal^3^` Helm charts to a version compatible with the `Edge 3.1` release. It follows the steps outlined in the <<day2-migration-mgmt-helm-upgrade-steps, "Upgrade Steps">> section.

_Use-case:_

* Current `Rancher` and `Metal^3^` charts need to be upgraded to an `Edge 3.1` compatible version.
...
. Locate the desired versions for `Rancher` and `Metal^3^` from the <<release-notes, release notes>>. For `Edge 3.1`, these versions would be `2.9.1` for _Rancher_ and `0.8.1` for _Metal^3^_.
...
==== SUSE Helm chart upgrade - non-EIB
...
This section provides an example of upgrading the `Rancher` and `Metal^3^` Helm charts to a version compatible with the `Edge 3.1` release. It follows the steps outlined in the <<day2-migration-mgmt-helm-non-eib, "SUSE Helm chart upgrade - non-EIB">> section.
...
. Locate the desired versions for `Rancher` and `Metal^3^` from the <<release-notes, release notes>>. For `Edge 3.1`, these versions would be `2.9.1` for Rancher and `0.8.1` for Metal^3^.
...
This section covers how to migrate your `Edge 3.0.X` downstream clusters to `Edge 3.1`.
...
==== Charts deployed through the Edge Image Builder
...
This means that, before migrating to an `Edge 3.1` compatible version, any Helm charts deployed on an `Edge 3.0` environment using EIB should have their chart manifests removed from the manifests directory of the relevant Kubernetes distribution.

...
This section describes how to perform an operating system upgrade using <<components-fleet,Fleet>> and the <<components-system-upgrade-controller,System Upgrade Controller (SUC)>>.
...
A different service is created depending on what upgrade your OS requires from one Edge version to another:
...
.. A link:https://en.opensuse.org/SDB:Zypper_usage#Updating_packages[normal package upgrade] - Done in order to ensure that all packages are with the latest version before the migration. Mitigating any failures related to old package version.
...
. Always upgrade the nodes *one* node at a time.
...
. *SCC registered machine* - All {cluster-type} cluster nodes should be registered to `https://scc.suse.com/` - Needed so that the systemd.service can successfully connect to the desired RPM repository.
+
[IMPORTANT]
====
...
As mentioned in the <<{cluster-type}-day2-fleet-os-upgrade-overview, Overview>> section, OS upgrades are done by shipping `SUC plans` to the desired cluster through one of the following ways:
...
In cases where custom changes are needed (e.g. add custom tolerations), users should refer the `os-upgrade` fleet from a separate repository, allowing them to add the changes to the SUC plans as required.
...
* `plan-control-plane.yaml` - SUC plan resource for *control-plane* nodes.

* `plan-worker.yaml` - SUC plan resource for *worker* nodes.

* `secret.yaml` - Secret that ships the `upgrade.sh` script, which is responsible for creating the <<{cluster-type}-day2-fleet-os-upgrade-components-systemd-service, systemd.service>>.

* `config-map.yaml` - ConfigMap that holds configurations that are consumed by the `upgrade.sh` script.
...
To better understand how your GitOps workflow can be used to deploy the *SUC Plans* for OS upgrade, it can be beneficial to take a look at the <<{cluster-type}-day2-fleet-os-upgrade-overview,overview>> section.

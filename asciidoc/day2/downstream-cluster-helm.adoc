...
An *example* for the `SUSE Storage` helm chart would look like:
...
[NOTE]
====
These are just example values that are used to illustrate custom configurations over the `SUSE Storage` chart. They should *NOT* be treated as deployment guidelines for the `SUSE Storage` chart.
====

...
Therefore, we recommend that you not only ensure that the driver version that you are selecting is compatible with your GPU and meets any CUDA requirements that you may have by checking the https://docs.nvidia.com/cuda/cuda-toolkit-release-notes/[CUDA release notes], but also to check that the driver version that you plan on deploying has a matching version in the http://download.nvidia.com/suse/sle15sp5/x86_64/[NVIDIA SLE15-SP5 repository] to ensure that you have equivalent package versions for the supporting components available. To find the NVIDIA open-driver versions, search the SUSE Customer Center for the "nvidia-open-driver" in https://scc.suse.com/packages?name=SUSE%20Linux%20Enterprise%20Micro&version=5.5&arch=x86_64[SLE Micro 5.5 for x86_64]. Here, you will see _four_ versions available, with _545.29.06_ being the newest:
...
Okay, so you have demonstrated full functionality of your applications and GPUs on SLE Micro and you now want to use https://github.com/suse-edge/edge-image-builder[SUSE Edge Image Builder] to provide it all together via a deployable/consumable ISO or RAW disk image. This guide does not explain how to use Edge Image Builder, but it provides the necessary configurations to achieve such a configuration. Below you can find an example of an image definition, along with the necessary Kubernetes configuration files, to ensure that all the required components are deployed out of the box. Here is the directory structure of the Edge Image Builder directory for the example shown below:
...
Besides this, we need to add additional components so they get loaded by Kubernetes at boot time. The EIB directory needs a `kubernetes` directory first, with subdirectories for the configuration, Helm charts and any additional manifests required:
...
We use the built-in Helm Controller to deploy the NVIDIA Device Plugin through Kubernetes itself. We provide the configuration, and it is applied at boot:

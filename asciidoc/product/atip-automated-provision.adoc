...
* `SUSE Linux Micro` as the OS. Depending on the use case, configurations like networking, storage, users and kernel arguments can be customized.
...
While this is the default configuration for the metal3 chart, ensure that this kernel parameter aligns with metal3 chart's `predictableNicNames` value.

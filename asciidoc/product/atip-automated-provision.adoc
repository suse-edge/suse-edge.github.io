...
[NOTE]
====
If no network configuration for the host has been specified, either at image build time or through the `BareMetalHost` definition, an autoconfiguration mechanism (DHCP, DHCPv6, SLAAC) will be used, for both IPv4 and IPv6. For more details or complex configurations, check the xref:advanced-network-configuration[].
====
...
[NOTE]
====
Make sure to run Metal3 version 0.9.0 or greater for dual-stack deployments, only single stack deployments are otherwise possible.
====
...
[NOTE]
====
* If no network configuration for the host has been specified, either at image build time or through the `BareMetalHost` definition, an autoconfiguration mechanism (DHCP, DHCPv6, SLAAC) will be used, for both IPv4 and IPv6. For more details or complex needs, check the xref:advanced-network-configuration[].
...
Any other `nmstate` compliant definition can be used to configure the network for the downstream cluster to adapt to the specific requirements, for instance it is possible to specify a static dual-stack configuration:

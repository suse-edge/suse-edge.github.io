...
$ podman run --rm \
  ...
  --debug
----
where `$\{MGMT_CLUSTER_REGISTRY_IP\}` is the reserved static IP given to the SUSE Private Registry instance deployed in the Management cluster, as described in the previous xref:mgmt-cluster-kubernetes-folder-airgap[section].

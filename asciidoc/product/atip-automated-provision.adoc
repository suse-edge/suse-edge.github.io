...
type: Opaque
data:Downstream cluster provisioning with Directed network provisioning
...
Following yaml file is an example configuration for the worker nodes: 
...
metadata:
  lapiVersion: infrastructure.cluster.x-k8s.io/v1beta1
kind: Metal3DataTemplate
metadata:
...
The `Metal3MachineTemplate` object contains `dataTemplate`, `hostSelector`, and `image` for the worker nodes:
...
Once the file is created by joining the previous blocks, runthe following command in the management cluster to start provisioning the new three bare-metal hosts:

...
The content of `custom/files/sriov-auto-filler.sh` is a script that can be used to configure the system for SR-IOV and can be downloaded from the following https://github.com/suse-edge/atip/blob/release-{version-edge-registry}/telco-examples/edge-clusters/dhcp/eib/custom/files/sriov-auto-filler.sh[link].

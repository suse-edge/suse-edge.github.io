...
PTP synchronization is enabled at provisioning time in two steps: enabling the
daemons in the EIB image and deploying the configuration files through Cluster API.
Only the configuration files change between the different PTP scenarios, so a single
generic example is used here and each scenario provides its specific values.
...
files and services on top of this base example.
====

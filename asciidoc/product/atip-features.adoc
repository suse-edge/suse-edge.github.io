...
==== Bond CNI
SUSE Edge is shipping the bond cni plugin. In general terms bonding provides a
...
The following is supported for the bond cni plugin with multus:
...
container. Here we have picked enp8s0 and enp9s0. Start by creating network
...
the host. These interfaces will be mapped through to the container. Here we have
picked enp8s0 and enp9s0. Start by creating network attachment definitions for
them: 

...
* *Grand Master (GM)*: the reference for the PTP domain (domain `24`), normally locked
...
. Verify that `ens6f0np0` locks to the time regenerated by the BC. The port moves to
...
chain stays traceable to the Grand Master. To see both identities explicitly, query the
...
Grand Master, forwarded unchanged through the Boundary Clock.
+
...
OC would be synchronizing directly from a Grand Master, with no Boundary Clock in the
path.
...
integrated controller receives PTP from the Grand Master and the add-in cards regenerate
...
Before going through the setup steps, download and install the latest kernel driver for
the _Intel 800 Series Network Devices_ from the
...
`ice-kmp-default-*sles16sp0.x86_64.rpm` RPM it contains, for example
...
* *Grand Master (GM)*: the reference for the PTP domain (domain `24`), on the upstream
segment.
* *PTP-aware switch*: acts as a Boundary Clock (T-BC) on the timing path; every port
...
----
+
Look up each device type with `lspci` to tell the integrated controller apart from the
add-in cards:
+
[,console]
----
...
_E830-CC_ add-in cards (`/dev/ptp2`, `/dev/ptp3`). Identify the administrative port of the
integrated controller, which manages its PHC:
+
[,console]
----
...
every PHC in the node ends up sharing the same time. Create its configuration file:
+
[#gnrd-ts2phc-config]
...
====
+
The integrated instance sets no `serverOnly` or `clientOnly` on its ports: it relies on the
BMCA to assign each role. With a GM reachable through one of them, the BMCA selects that port
(`em2`) as the Time Receiver (`SLAVE`) and disciplines `/dev/ptp1`, while the remaining ports
follow the BMCA outcome. This is what makes the node a Boundary Clock rather than a plain
receiver, so the ports must stay unconstrained: forcing `clientOnly` would stop it from
forwarding time, and forcing `serverOnly` would stop it from locking to the GM. The add-in
cards, in contrast, force `serverOnly 1` because they must always be Time Transmitters — they
take their time from the DPLL and `ts2phc`, not by receiving PTP, so they must never be
elected as receivers. If tighter control is needed, add the `serverOnly` or `clientOnly` flag
explicitly under each interface.
+
...
drives which port the BMCA elects, as reflected in the port states reported by `ptp4l`.
+
...
. Forward the Grand Master parameters across the `ptp4l` instances.
+
...
----
#!/bin/bash
# Retrieve Grand Master information via pmc for the given ptp4l config file
set -euo pipefail
...
[Unit]
Description=Forward Grand Master parameters to the add-in ptp4l instances
...
[Unit]
Description=Refresh forwarded Grand Master parameters
...
then propagate to the downstream OC). For example, `gm.ClockClass` changes:
+
[,console]
----
...
instances, the `ts2phc` service, the DPLL boot-time setup and the Grand Master forwarding

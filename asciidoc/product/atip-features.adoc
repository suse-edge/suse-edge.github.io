...
* Enable Predictable Network Interface Naming via `net.ifnames=1`. This parameter must be explicitly enabled, as it is not configured by default in SUSE Linux Micro versions prior to 6.2. This is aligned with the configuration in the Management Cluster's Metal3 chart to ensure Directed Network Provisioning method to work. Ensuring consistent interface naming is also critical if SR-IOV is utilized.

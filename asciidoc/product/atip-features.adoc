...
* <<cpu-pinning,CPU Pinning>>: Isolating the CPUs via kernel arguments, TuneD profile, and Kubernetes configuration.
...
* Isolate the CPU cores using `isolcpus`, `nohz_full`, `rcu_nocbs`, and `irqaffinity`. For a comprehensive list of CPU pinning techniques, refer to  <<cpu-pinning,CPU Pinning>> chapter.
...
==== Kubernetes Configuration for CPU Pinning
...
----
+
* The `FEC` is `intel.com/intel_fec_5g` and the value is 16.
...
* Configure the acclerator card and its virtual functions with a 5G profile. A unique VF token (UUID) must be provided. The workload would consume this VF token to utilize the card's FEC acceleration capabilities:

[,shell]
----
$ pf_bb_config ACC100 -c /opt/pf-bb-config/acc100_config_vf_5g.cfg -v 00112233-4455-6677-8899-aabbccddeeff

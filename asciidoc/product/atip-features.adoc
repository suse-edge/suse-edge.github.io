...
* *Hardware*: the `SDP1` input pin of each add-in PHC must be enabled so the on-board DPLL
can deliver the 1PPS pulse to it. This is done in the DPLL setup script of the previous step
...
Without the pin enablement the pulse never reaches the PHC; without the `ts2phc` configuration

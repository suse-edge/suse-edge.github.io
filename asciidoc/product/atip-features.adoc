...
signalling interfaces by HTTP/2) but it is still required inside the 5G RAN domain (across the network elements making up a "disaggregated" gNodeB instance
and in the Xn-C interface connecting gNodeB instances for signaling purposes) and as the transport layer in the N2 (aka NG-C) reference point used to connect
gNodeB control plane instances (RAN domain) with AMF instances (Core Nework domain).

It is then required to enable the `sctp` linuk kernel module (`lksctp`) in downstream clusters' nodes hosting SCTP-capable Telco workloads leveraging it:

* To manually load the `sctp` kernel module in a linux node:
...
* To automate the loading of the `sctp` kernel module at each node boot, create a file named `sctp.conf` in `/etc/modules-load.d/` directory which simply contains
 the string `sctp` (i.e., the name of the kernel module that https://www.freedesktop.org/software/systemd/man/latest/modules-load.d.html[systemd-modules-load.service]
 will load at boot).
...
The easiest and best way to assure to load the `sctp` kernel module at each node boot is to add that `/etc/modules-load.d/sctp.conf` file to the
Edge Image Builder (EIB) built raw image used to provision downstream nodes; see the xref:../quickstart/eib.adoc#adding-operating-system-files[EIB documentation] for more information on how to do it.

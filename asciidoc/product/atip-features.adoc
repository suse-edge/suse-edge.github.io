...
//
// ============================================================
// New PTP documentation structure (to be filled in). Replaces
// the sections marked "-old" below, which are kept as a temporary
// backup until the rewrite is complete.
// Anchor swap pending: when the "-old" block is removed, move the
// canonical anchors (e.g. ptp-configuration) onto these sections.
// ============================================================
//
...
* *Grand Master (GM)*: the primary source of time for the domain. It is normally
...
clock to the PHC. On a Grand Master, `ts2phc` additionally steers the PHC from the
...
with the network driver. How much of the timing can be controlled depends on
whether the NIC integrates a manageable DPLL:
...
as an optional frequency enhancement. In both cases the Grand Master derives its
...
image::product-atip-ptp-overview.png[PTP-only distribution with Grand Master, Boundary Clock and Ordinary Clocks,scaledwidth=100%]

.Time and frequency distribution with PTP and SyncE (optional enhancement)
image::product-atip-ptp-synce-overview.png[PTP and SyncE distribution with Grand Master, Boundary Clock and Ordinary Clocks,scaledwidth=100%]
...
* <<ptp-scenario-oc-synce,OC with PTP + SyncE>>
* <<ptp-scenario-bc-single-nic,BC with PTP + propagation (single NIC)>>
* <<ptp-scenario-bc-multi-nic-dpll,BC with PTP + propagation (multi-NIC, DPLL)>>
...
`ts2phc.service`). Together they run the protocol and keep the different clocks
aligned:
...
signal, such as the PPS delivered by a GNSS receiver. It is used on a Grand Master
...
Enable the daemons required by each node through the `systemd` section of the
manifest, once their configuration files are in place (see the scenarios below).
====
...
* *Grand Master (GM)*: the upstream reference for the PTP domain (domain `24` for
...
| `$\{PTP4L_CONF_CONTENTS\}`
| The `ptp4l` file shown in <<ptp-scenario-oc-model1,Model 1>>.
...
| `$\{PTP4L_CONF_CONTENTS\}`
| The `ptp4l` file shown in <<ptp-scenario-oc-model2,Model 2>>.
...
// TODO: image::<path>[OC with PTP + SyncE]

*Configuration*

// TODO

*CAPI automation*: see <<ptp-capi-oc-synce,the CAPI automation for this scenario>>.

[#ptp-scenario-bc-single-nic]
==== BC with PTP + propagation (single NIC)
...
// TODO: image::<path>[BC with PTP + propagation (single NIC)]

*Configuration*

// TODO

*CAPI automation*: see <<ptp-capi-bc-single-nic,the CAPI automation for this scenario>>.

[#ptp-scenario-bc-multi-nic-dpll]
==== BC with PTP + propagation (multi-NIC, DPLL)

*Explanation*

// DRAFT (from notes) — to be expanded.
On an Intel Granite Rapids-D (GNR-D) node, the Boundary Clock spans several NICs
and the timing is distributed internally through the on-board DPLL (timing
module), without external cabling:

. `ptp4l` disciplines the PHC of the integrated controller from the upstream Grand
Master.
. The integrated PHC feeds the timing module (DPLL) with 1PPS and 1kPPS signals
through the SDP0 and SDP2 pins.
. The DPLL synchronizes the secondary NICs (add-in cards) to that timing, and
`ts2phc` keeps their PHCs aligned with the integrated PHC.
. Each NIC then propagates PTP downstream to the Radio Units (RU) over its
fronthaul ports.

*Diagram*

// TODO: image::<path>[BC with PTP + propagation (multi-NIC, DPLL)]

*Configuration*

// TODO

*CAPI automation*: see <<ptp-capi-bc-multi-nic-dpll,the CAPI automation for this scenario>>.
...
*CAPI automation*: see <<ptp-capi-bc-synce-multi-nic-dpll,the CAPI automation for this scenario>>.

[#ptp-cluster-api]
=== Cluster API integration

// TODO: Describe how the scenarios above are applied automatically at
// provisioning time through Cluster API (registration/config files,
// systemd units, etc.).

[#ptp-capi-oc]
==== Scenario 1 - Ordinary Clock (PTP-only)

// TODO: CAPI automation for <<ptp-scenario-oc>>.

[#ptp-capi-oc-synce]
==== OC with PTP + SyncE

// TODO: CAPI automation for <<ptp-scenario-oc-synce>>.

[#ptp-capi-bc-single-nic]
==== BC with PTP + propagation (single NIC)

// TODO: CAPI automation for <<ptp-scenario-bc-single-nic>>.

[#ptp-capi-bc-multi-nic-dpll]
==== BC with PTP + propagation (multi-NIC, DPLL)

// TODO: CAPI automation for <<ptp-scenario-bc-multi-nic-dpll>>.

[#ptp-capi-bc-synce-multi-nic-dpll]
==== BC with PTP + SyncE + propagation (multi-NIC, DPLL)

// TODO: CAPI automation for <<ptp-scenario-bc-synce-multi-nic-dpll>>.

//
// ============================================================
// BACKUP — original PTP content (to be removed after rewrite).
// ============================================================
//

[#ptp-configuration]
== Precision Time Protocol -old
...
=== Install PTP software components -old
...
[#ptp-telco-config]
=== Configure PTP for telco deployments -old
...
==== PTP profile ITU-T G.8275.1 -old
...
==== PTP profile ITU-T G.8275.2 -old
...
[#ptp-bc-setup]
==== PTP configuration of a Boundary Clock -old
...
[#ptp-phc2sys-setup]
==== Synchronization of the system clock from PTP -old
...
=== PTP on Intel Granite Rapids-D platforms -old
...
===== `ptp4l` configuration file for the integrated controller (`/dev/ptp1`) -old
...
===== `ptp4l` configuration file for add-in cards (`/dev/ptp2` or `/dev/ptp3`) -old
...
===== `ts2phc` configuration file for add-in cards -old
...
[#gnrd-pmc-forward]
===== GM parameters forwarding script -old

...
* *Runtime stack*: This is the part of ATIP that is used to run the workloads.
  ** Kubernetes with secure and lightweight distributions like <<components-k3s,K3s>> and <<components-rke2,RKE2>> (`RKE2` is secure hardened, certified, and optimized for government use and regulated industries)
  ...
  ** Optimized Operating System with <<components-slmicro,SLE Micro>> to enable a secure, lightweight, and immutable (transactional filesystem) OS for running containers. SLE Micro is available on `aarch64` and `x86_64` architectures, and it also supports `Real-Time Kernel` for Telco and edge use cases.
...
Using the <<components-eib,Edge Image Builder>> to create a new `ISO` image with the management stack included. You can then use this `ISO to install a new management cluster on VMs or Bare metal.
...
NOTE: For more information about `MetalLB`, see  <<components-metallb,look here:>>

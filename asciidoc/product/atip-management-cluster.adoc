...
where `version` is the version of Kubernetes to be installed. In our case, we are using a RKE2 cluster, so the version has to be less than 1.29 to be compatible with `Rancher` (e.g `v1.28.8+rke2r1`).

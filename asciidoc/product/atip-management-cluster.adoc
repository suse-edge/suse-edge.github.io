...
. **xref:mgmt-cluster-image-preparation-airgap[Image preparation for air-gap environments]**: The step is to show the differences to prepare the manifests and files to be used in an air-gap scenario.
  ...
  - xref:mgmt-cluster-custom-folder-airgap[Modifications in the custom folder]: The `custom` folder must be modified to include the resources needed to run the management cluster in an air-gap environment.
        ** Register script: The `custom/scripts/99-register.sh` script must be removed when use an air-gap environment.
...
The directory structure for air-gap environments is the same as for connected environments, with the following differences will be explained below:
...
The `clusterctl.yaml` file contains the configuration to specify the images location and the overrides to be used by the `clusterctl` tool. The `overrides` folder contains yaml files manifests to be used instead of downloading them from the internet.

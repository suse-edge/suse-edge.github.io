...
[NOTE]
====
Currently only a single IPv4 address can be used for VIP addresses. This limitation will be removed in a future release.
====
...
* `$\{MGMT_GATEWAY_V4/6\}`: The gateway IPv4/6 address.
* `$\{MGMT_DNS_IP_V4/6\}`: The DNS server IPv4/6 address.
* `$\{MGMT_MAC\}`: The MAC address of the network interface.
* `$\{MGMT_NODE_IP_V4/6\}`: The IPv4/6 address of the management cluster.
...
If you want to use autoconfiguration to get the IP address (DHCP, DHCPv6, SLAAC), you can use the following configuration (the `MAC` address must be set properly using the `$\{MGMT_MAC\}` variable):
...
- Currently the connectivity to the host `BMC` as well as the communication between the management cluster and the Ironic Python Agent will still run on IPv4 only. This limitation will be removed in a future release.
====

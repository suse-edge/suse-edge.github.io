...
Some containers part of the stack leverage the host networking, modify the network configuration for the host(s) under the `network` directory, to enable IPv6 connectivity:
...
Fill the file with the the gateway IP addresses, additional DNS server (if needed), the MAC address of the network interface and the the IP addressed of the management cluster. If address autoconfiguration is preferred instead, refer to the following excerpt instead and just set the `$\{MGMT_MAC\}` variable:
...
We can now move to a sample (single node) configuration for the first option, by editing `kubernetes/helm/values/metal3.yaml` to look like:
...
Alternatively, to use the hostnames in place of IP addresses, modify `kubernetes/helm/values/metal3.yaml` to:
...
and `kubernetes/helm/values/rancher.yaml`

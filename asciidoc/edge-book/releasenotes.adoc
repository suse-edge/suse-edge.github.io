...
SUSE Edge "3.2" is supported for 18-months of production support, with an initial 6-months of GA\*, followed by 12-months of EOM\**. During the GA phase, SUSE may introduce new features (that do not break existing functionality), introduce bug fixes, and deliver security patches. During the EOM phase, only critical security and bug fixes will be introduced, with other fixes delivered at our discretion until it reaches its EOL\*** date.

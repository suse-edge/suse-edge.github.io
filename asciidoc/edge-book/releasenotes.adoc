...
To confirm if your cluster is affected by the issue, please execute following command:
[,bash]
----
...
If this happens, please remove `inject-ca-from` annotation from following CRDs:
[,yaml]
----
...
----
Removal could be done with command:
[,bash]
----
...
To fix this, please edit `/etc/rancher/rke2/config.yaml` to increase limit for the etcd storage:
[,yaml]
----
etcd-arg:
  - "quota-backend-bytes=8589934592"
----
In the example, storage is set to 8GB. After editing the file, rke2-server restart is required.
This increase should be only done for maintenance purposes. After applying the fix from previous paragraph and confirming that the issue is no longer present, it is better to revert the configuration to original value (default for etcd storage is 2GB)

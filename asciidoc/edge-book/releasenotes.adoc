...
* Some container repositories are currently reachable via IPv4 only, for this reason a local registry on the Management Cluster is required for an IPv6 only downstream cluster.

* When using SR-IOV Virtual Functions in pods on a system with a realtime kernel
  the system can become unresponsive. This specifically happens when deleting a
  pod that is using SR-IOV Virtual Functions. In this situation the journal logs
  a message like "rtmutex deadlock detected". This has been noticed on kernel
  version 6.12.0-160000.9-rt. The solution is to upgrade to the latest kernel
  version.

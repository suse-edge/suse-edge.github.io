...
* `HelmChartConfigs` manifests may fail if they are put in the `/kubernetes/manifests` section in the configuration directory. The best practice is to place any `HelmChartConfigs` in `/var/lib/rancher/{rke2/k3s}/server/manifests/` using os-files (operating-system-files).
...
s| Rancher Turtles (CAPI) | 0.21.0 | 303.0.5+up0.21.0 | *registry.suse.com/edge/charts/rancher-turtles:303.0.5_up0.21.0* +

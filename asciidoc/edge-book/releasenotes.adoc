...
* Updated to SUSE Linux Micro 6.2
* Metal3 Ironic Python Agent (IPA) is aligned with the same codebase as SUSE Linux Micro 6.2 (including the Kernel)
* Updated Elemental to 1.8.0
* TODO
...
* Running https://documentation.suse.com/suse-edge/3.4/html/edge/atip-features.html#ptp-telco-config[PTP feature] in default configuration requires additional steps:
Following needs to be added to overrides in the ptp4l service for it to work correctly in Edge 3.5 release:
[,bash]
----
[Service]
PrivateDevices=

...
Availability Date: 17th April 2025
...
* An SELinux issue, identified as a missing backport in the SUSE Linux Micro kernel, prevents the standard RKE2 1.31.7 `ingress-nginx` image from being used successfully. A workaround must be applied to use a modified version of the image, that applies the required fixes for https://nvd.nist.gov/vuln/detail/CVE-2025-1974[CVE-2025-1974]. The following configuration must be applied to any RKE2 clusters:
...
spec:
  valuesContent: |-
    controller:
      image:
        tag: v1.12.0-hardened11
----

If you wish to directly apply this workaround to a running system, this can be applied via a simple `kubectl apply`, following an update to SUSE Edge 3.2.1, and the system should reconcile to a successful state. However, for management at scale, it would be advised to apply this change via Rancher's Fleet implementation by https://fleet.rancher.io/bundle-add[creating a bundle], or add it to your existing https://fleet.rancher.io/gitrepo-add[GitOps repo].

However, if you're creating new images via Edge Image Builder, you can apply the workaround by creating the necessary override before creating your image, as follows:
...
NOTE: The requirement for the above workaround will be removed in the coming weeks with updates to SUSE Linux Micro, and as part of a further release of SUSE Edge 3.2.z. This issue does *not* impact K3s.

...
* With the included Metal3 version, it is now possible to define dual-stack CAPI clusters. Previously only single stack deployments were possible due to a bug that filtered additional CIDRs for both Pods and Services. Please refer to https://github.com/suse-edge/atip[https://github.com/suse-edge/atip] for examples on how to configure IPv4, IPv6 and dual-stack downstream clusters (currently limited to single host deployments)

== Known issues

* A well-known bug affects clusters using static IPs on a network where DHCP servers and/or RAs are also present: any static network configuration will only apply to the provisioned host and will not be in effect during the host discovery and enrollment phase. Please refer to the https://github.com/suse-edge/atip/tree/main/telco-examples/edge-clusters/dhcp-less/dual-stack/single-node#readme[ATIP repository] for more details and updates.
...
| Rancher Prime | 2.10.1 | 2.10.1 | https://github.com/rancher/rancher/releases/download/v2.10.1-alpha1/rancher-images.txt[Rancher 2.10.1 Images] TODO: Update Rancher with correct version once it is released +
 https://charts.rancher.com/server-charts/prime[Rancher Prime Helm Repo]
| Longhorn | 1.7.2 | 105.1.0+up1.7.2 | https://raw.githubusercontent.com/longhorn/longhorn/v1.7.2/deploy/longhorn-images.txt[Longhorn 1.7.2 Images] +
...
| NeuVector| 5.4.1 | 105.0.0+up2.8.3 | registry.suse.com/rancher/mirrored-neuvector-controller:5.4.1 +

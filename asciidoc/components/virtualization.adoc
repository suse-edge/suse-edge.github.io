...
Next, proceed to install the *KubeVirt* and *Containerized Data Importer (CDI)* Helm charts:

[,shell]
----
$ helm upgrade -i kubevirt oci://registry.suse.com/edge/kubevirt-chart --namespace kubevirt-system --create-namespace
$ helm upgrade -i cdi oci://registry.suse.com/edge/cdi-chart --namespace kubevirt-system --create-namespace
----
...
See xref:rancher-dashboard-extensions.adoc[Rancher Dashboard Extensions section] for installation guidance.

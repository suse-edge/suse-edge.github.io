...
In the example environment, another openSUSE Tumbleweed virtual machine is deployed, cloud-init is used to install nginx as a simple web-server at boot time, and a very simple message is configured to be returned to verify that it's working as expected when a call is made. If you want to see how this is done, simply `base64 -d` the cloud-init section in the output below.

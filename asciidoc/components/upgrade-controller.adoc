...
A Kubernetes controller capable of performing upgrades over the following {product} platform components:
...
[NOTE]
====
The `Upgrade Controller` currently supports {product} platform upgrades only for *non air-gapped management* clusters. Refer to the <<components-upgrade-controller-known-issues>> section for more information.
====

== How does {product} use Upgrade Controller?

The *Upgrade Controller* is essential in automating the (formerly manual) "Day 2" operations required to upgrade management clusters from one {product} release version to the next.
...
While it supports some "Day 2" operations for the {product} platform, it *does not* cover all of them. Moreover, even for supported operations, users have to manually configure, maintain, and deploy multiple `SUC Plans` — an error-prone process that can lead to unexpected issues.

This led to the need for a tool that **automates** and **abstracts** the complexity of managing various "Day 2" operations for the {product} platform. Thus, the `Upgrade Controller` was developed. It simplifies the upgrade process by introducing a single `user-facing resource` that drives the upgrade. Users only need to manage this resource, while the `Upgrade Controller` takes care of the rest.
...
As an alternative to the manual installation described above, it is possible to install the upgrade controller as part of the initial deployment orchestrated by <<components-eib, Edge Image Builder>>.
...
Charts will be upgraded sequentially based on their order in the `ReleaseManifest`. Additional values can also be passed through the `UpgradePlan`. If a chart's version remains unchanged in the new {product} release, it will not be upgraded. For more information about this, refer to <<components-upgrade-controller-extensions-upgrade-plan>>.

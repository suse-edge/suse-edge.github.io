...
SUSE Storage is a lightweight, reliable and user-friendly distributed block storage system designed for Kubernetes.
As an open source project, SUSE Storage was initially developed by Rancher Labs and is currently incubated under the CNCF.
...
[TIP]
====
For additional help installing open-iscsi, refer to the {link-suse-storage-iscsi}[official SUSE Storage documentation].
====

=== Installing SUSE Storage

There are several ways to install SUSE Storage on your Kubernetes clusters.
This guide will follow through the Helm installation, however feel free to follow the {link-suse-storage-installation}[official documentation] if another approach is desired.
...
----
helm install longhorn-crd rancher-charts/longhorn-crd --namespace longhorn-system --create-namespace --version {version-suse-storage-crd-chart}
helm install longhorn rancher-charts/longhorn --namespace longhorn-system --version {version-suse-storage-chart}
----
+
...
however, we suggest to familiarize yourself with the {link-suse-storage-terminology}[terminology documentation] that Longhorn offers.
====
...
kubernetes:
  ...
  helm:
    charts:
      - name: longhorn
        version: {version-suse-storage-chart}
        repositoryName: longhorn
        targetNamespace: longhorn-system
        createNamespace: true
        installationNamespace: kube-system
      - name: longhorn-crd
        version: {version-suse-storage-crd-chart}
        repositoryName: longhorn
        targetNamespace: longhorn-system
        createNamespace: true
        installationNamespace: kube-system

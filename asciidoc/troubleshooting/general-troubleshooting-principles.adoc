...
* *Documentation*: Always refer to the official https://documentation.suse.com/suse-edge/[{product} documentation] and also upstream documentation for detailed information.
* *Versions*: {product} is an opinionated and thoroughly tested version of different SUSE components. The versions of each component per {product} release can be observed in the https://documentation.suse.com/suse-edge/support-matrix/html/support-matrix/index.html[{product} support matrix].
* *Known issues*: For each {product} release there is a “Known issues” section on the release notes that contains information of issues that will be fixed on future releases but can affect the current one.

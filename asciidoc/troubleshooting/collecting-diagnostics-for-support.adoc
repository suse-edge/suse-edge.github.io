...
* *Component versions*: {product} version, components versions (RKE2/K3, EIB, Metal^3^, Elemental,..).
* *Relevant logs*: 
...
* *For Edge (Nessie)*: Nessie {version-nessie} is a powerful diagnostic tool designed to collect logs and configuration data from {product} environments. It gathers comprehensive information from both the host system and Kubernetes clusters, making it invaluable for troubleshooting and support. 
** Nessie has two "modes" a kubernetes mode and a system mode.
*** To collect logs from a {product} cluster, run (provided that you have access to the kubeconfig file locally):
+

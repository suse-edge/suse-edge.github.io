...
**NOTE:** For a real HA scenario, it is required for `etcd` to have an odd number of nodes, so it would be required to add two more control plane nodes.
...
```
kubectl port-forward $(kubectl get pods --selector "app.kubernetes.io/name=traefik" -o=name -n kube-system) -n kube-system 9000:9000
```

Then, browse [http://localhost:9000/dashboard](http://localhost:9000/dashboard) to observe the Traefik dashboard:

![](./assets/traefikdashboard.png)

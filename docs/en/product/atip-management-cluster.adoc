...
An example of the directory structure can be found in the https://github.com/suse-edge/telco-cloud-examples[{product} GitHub repository under the "telco-examples" folder].
====
...
embeddedArtifactRegistry:
  ...
  images:
    ...
    - name: registry.rancher.com/rancher/hardened-cluster-autoscaler:v1.10.3-build20260717
    ...
    - name: registry.rancher.com/rancher/mirrored-cilium-cilium:v1.19.6
    - name: registry.rancher.com/rancher/mirrored-cilium-operator-generic:v1.19.6
    ...
    - name: dp.apps.rancher.io/containers/kubernetes-csi-node-driver-registrar:2.17.0-17.6
    ...
    - name: registry.rancher.com/rancher/neuvector-compliance-config:1.0.16
    - name: registry.rancher.com/rancher/neuvector-controller:5.6.1
    - name: registry.rancher.com/rancher/neuvector-enforcer:5.6.1
    - name: registry.rancher.com/rancher/neuvector-updater:0.0.14
    - name: registry.rancher.com/rancher/nginx-ingress-controller:v1.14.5-hardened2
    - name: registry.suse.com/rancher/cluster-api-addon-provider-fleet:v0.15.0
    ...
    - name: registry.rancher.com/rancher/cluster-api-controller:v1.13.3
    - name: registry.suse.com/rancher/cluster-api-provider-metal3:v1.13.2
    - name: registry.suse.com/rancher/cluster-api-provider-rke2-bootstrap:v0.25.0
    - name: registry.suse.com/rancher/cluster-api-provider-rke2-controlplane:v0.25.0
    - name: registry.rancher.com/rancher/ip-address-manager:v1.13.2
...
where the IP is the same as the previously configured `$\{MGMT_CLUSTER_REGISTRY_IP\}`, and the `&lt;USERNAME&gt;`, and `&lt;PASSWORD&gt;` values can be retrieved from the https://documentation.suse.com/cloudnative/suse-private-registry/html/private-registry/pr-deployment.html#pr-deployment-kube-secrets[SUSE Private Registry official documentation]; the `&lt;AUTH&gt;` value set in the `auth` field is the base64 encoding of the `&lt;USERNAME&gt;:&lt;PASSWORD&gt;` concatenation.
...
For more information visit https://github.com/suse-edge/telco-cloud-examples/tree/main/telco-examples/mgmt-cluster/dual-stack[{product} GitHub repository under the "dual-stack" folder], where an example directory structure can be found.

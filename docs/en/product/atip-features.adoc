...
* `drivers`: `vfio-pci`
* `pfNames`: `eno16595np0` (physical interface name)
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
      - name: kube-sriovdp
        image: registry.suse.com/edge/3.7/sriov-network-device-plugin:v3.10.0
        imagePullPolicy: IfNotPresent
        args:
        - --log-dir=sriovdp
        - --log-level=10
...
===================================
0000:19:00.0 'BCM57504 NetXtreme-E 10Gb/25Gb/40Gb/50Gb/100Gb/200Gb Ethernet 1751' if=eno8303np0 drv=bnxt_en unused=igb_uio,vfio-pci *Active*
0000:19:00.1 'BCM57504 NetXtreme-E 10Gb/25Gb/40Gb/50Gb/100Gb/200Gb Ethernet 1751' if=eno8403np1 drv=bnxt_en unused=igb_uio,vfio-pci
0000:19:00.2 'BCM57504 NetXtreme-E 10Gb/25Gb/40Gb/50Gb/100Gb/200Gb Ethernet 1751' if=eno8503np2 drv=bnxt_en unused=igb_uio,vfio-pci
0000:19:00.3 'BCM57504 NetXtreme-E 10Gb/25Gb/40Gb/50Gb/100Gb/200Gb Ethernet 1751' if=eno8603np3 drv=bnxt_en unused=igb_uio,vfio-pci
...
| hugepagesz
| 1G
| This option allows to set the size of "huge pages" to 1 GB

| hugepages
| 40
| This is the number of "huge pages" requested to allocate at boot for the size defined just before (1 GB)

| hugepagesz
| 2M
| This option allows to set the size of "huge pages" to 2 MB

| hugepages
| 0
| This is the number of "huge pages" requested to allocate at boot for the size defined just before (2 MB)

| default_hugepagesz
| 1G
| This is the default value for the "huge pages" size
|===
...
* <<ptp-scenario-oc,Scenario 1a - Ordinary Clock (PTP-only)>>
* <<ptp-scenario-oc-synce,Scenario 1b - Ordinary Clock (Hybrid mode: PTP & SyncE)>>
* <<ptp-scenario-bc-single-nic,Scenario 2a - Boundary Clock (PTP-only) with Multiport Single NIC including propagation to Ordinary Clock server>>
* <<ptp-scenario-bc-synce-single-nic,Scenario 2b - Boundary Clock (Hybrid mode: PTP & SyncE) with Multiport Single NIC including propagation to Ordinary Clock server>>
* <<ptp-scenario-bc-multi-nic-dpll,Scenario 3a - Boundary Clock (PTP-only) Multi-NIC with internal timing module (DPLL GNR-D based)>>
* <<ptp-scenario-bc-synce-multi-nic-dpll,Scenario 3b - Boundary Clock (Hybrid mode: PTP & SyncE) Multi-NIC with internal timing module (DPLL GNR-D based)>>

[NOTE]
====
Only <<ptp-scenario-oc,Scenario 1a>> shows the configuration for both telco profiles,
ITU-T G.8275.1 and ITU-T G.8275.2. The remaining scenarios focus only on the ITU-T G.8275.1
...
[,console]
----
# ethtool -T eno16495np0
Time stamping parameters for eno16495np0:
Capabilities:
        hardware-transmit
        software-transmit
        hardware-receive
...
the interface has no PHC). Replace `eno16495np0` with the interface you intend to use for
PTP.
...
[#ptp-scenario-oc]
=== Scenario 1a - Ordinary Clock (PTP-only)
...
image::product-atip-ptp-scenario-oc.png[Scenario 1a - Ordinary Clock (PTP-only) with G.8275.1,scaledwidth=100%]

Configure the node as follows:

. Identify the PHC index `<N>` of the interface used for PTP (here `eno8403np1`); the value of the
printed field `PTP Hardware Clock` (which may also appear as `Hardware timestamp provider index`) 
gives the `/dev/ptp<N>` device that `ptp4l` disciplines and that `phc2sys` reads as its source:
+
[,console]
----
# ethtool -T eno8403np1
Time stamping parameters for eno8403np1:
Capabilities:
        hardware-transmit
        hardware-receive
        hardware-raw-clock
...
----
+
In this example the PHC of `eno8403np1` is `/dev/ptp1`.

. Create the `ptp4l` configuration file `/etc/ptp4l-G.8275.1.conf`. The interface is
declared as a section (`[eno8403np1]`), so the daemon does not need the `-i` flag:
+
[#ptp-conf-oc-g8275-1]
...
ptp4l[...]: "ptp4l" port 1 (eno8403np1): UNCALIBRATED to SLAVE on MASTER_CLOCK_SELECTED
...
image::product-atip-ptp-scenario-oc-g8275-2.png[Scenario 1a - Ordinary Clock (PTP-only) with G.8275.2,scaledwidth=100%]

Configure the node as follows:

. Identify the PHC of the PTP interface as in Model 1 (`ethtool -T eno8403np1`); in this
example the PHC of `eno8403np1` is `/dev/ptp1`.
...
=== Scenario 1b - Ordinary Clock (Hybrid mode: PTP & SyncE)

This scenario is not yet supported; it will be included in a future {product} release.

[#ptp-scenario-bc-single-nic]
=== Scenario 2a - Boundary Clock (PTP-only) with Multiport Single NIC including propagation to Ordinary Clock server
...
segments. Its `eno8403np1` port faces the *upstream* segment as a Time Receiver (SLAVE) and
disciplines the NIC's PHC; `eno8503np2` (and `eno8603np3`) face the *downstream* segment as Time
...
selects the GM as master on `eno8403np1`, which moves to `SLAVE` and disciplines the PHC.
`eno8503np2` and `eno8603np3` are forced to `serverOnly 1`, so they act only as Time Transmitters.
...
`eno8403np1` port, and a *downstream* segment between its `eno8503np2` port and the OC. These are kept
...
As in <<ptp-scenario-oc,Scenario 1a>>, this scenario uses the non-forwardable multicast
...
image::product-atip-ptp-scenario-bc-single-nic.svg[Scenario 2a - Boundary Clock (PTP-only) with Multiport Single NIC including propagation to Ordinary Clock server,scaledwidth=100%]
...
. Identify the PHC of the PTP NIC (`ethtool -T eno8403np1`); on a single NIC all ports share
the same PHC. In this example it is `/dev/ptp1`.

. Create `/etc/ptp4l-G.8275.1.conf`. The receiving port `[eno8403np1]` has no role flag, so
the BMCA selects it as Time Receiver; `[eno8503np2]` and `[eno8603np3]` are set to `serverOnly 1` to
...
. Enable and start the services and verify that `eno8403np1` is locked to the GM. The
...
[#ptp-scenario-bc-synce-single-nic]
=== Scenario 2b - Boundary Clock (Hybrid mode: PTP & SyncE) with Multiport Single NIC including propagation to Ordinary Clock server

This scenario is not yet supported; it will be included in a future {product} release.

[#ptp-scenario-bc-multi-nic-dpll]
=== Scenario 3a - Boundary Clock (PTP-only) Multi-NIC with internal timing module (DPLL GNR-D based)
...
different NICs internally, no longer requiring external cabling to keep all NIC PHCs in sync.
The overall deployment is thus simplified, but specific steps and software components are
required, such as boot-time DPLL and internal signal configuration.

This scenario extends <<ptp-scenario-bc-single-nic,Scenario 2a>> to a Boundary Clock (BC)
...
[IMPORTANT]
====
In this {product} release this scenario is only supported for the "default" (i.e., non "PREEMPT_RT") kernel flavour.
Support for the "RT" kernel flavour will be added in a following release.
====
...
the GM on `eno8403np1` and disciplines the integrated PHC (`/dev/ptp1`). The on-board DPLL
...
example on `eno16495np0`), acting as a Time Transmitter.
...
`eno8403np1`. The BMCA selects the GM as master, the port moves to `SLAVE` and `ptp4l`
...
manages the PHC. Inspect `/sys/class/ptp/ptp1/device/net/` to find it (for example `eno8703np0`);
it is the port used to configure the DPLL signals, and it is not necessarily the port that
carries PTP (`eno8403np1`).
====
...
image::product-atip-ptp-scenario-bc-multi-nic-dpll.svg[Scenario 3a - Boundary Clock (PTP-only) multi-NIC with internal timing module (DPLL GNR-D based),scaledwidth=100%]
...
<<ptp-scenario-bc-single-nic,Scenario 2a>>.
...
administrative port found above (`eno8703np0`) and the correct `ptpN` indices:
+
[#gnrd-startup-setup]
...
# The admin port that manages the PHC is under /sys/class/ptp/ptp1/device/net/ (e.g. eno8703np0).
# Only one port of the integrated E825-C NIC exposes it.

# Let the DPLL drive the Ethernet clock on the integrated controller
echo 4 1 > /sys/class/net/eno8703np0/device/tspll_cfg
...
reference is the PHC from the integrated NIC (`/dev/ptp1`, already disciplined by the GM through its
...
====
+
Each add-in card is configured through a section named after its first port (`[eno16495np0]`,
`[eno16595np0]`), which identifies the PHC to discipline. The parameters map the DPLL pulse to
that card:
+
...
====
+
The integrated instance declares only `eno8403np1`, the port that receives PTP from the GM, and sets
no `serverOnly` or `clientOnly` on it: it relies on the BMCA to assign the role. With a GM
reachable on `eno8403np1`, the BMCA selects it as the Time Receiver (`SLAVE`) and disciplines
...
[NOTE]
====
As in <<ptp-scenario-bc-single-nic,Scenario 2a>>, every node on the timing path uses the same
...
=== Scenario 3b - Boundary Clock (Hybrid mode: PTP & SyncE) Multi-NIC with internal timing module (DPLL GNR-D based)

...
This creates the output RAW image file named `eibimage-output-telco.raw`, based on the definition described above.
...
$ podman run --rm \
  -v ./:/opt:z \
  registry.suse.com/edge/{version-edge-registry}/release-manifest:{version-release-manifest} \
  mirror \
  -o /opt/output \
...
| hugepagesz | 1G    | Allows to set the size of "huge pages" to 1 GB.
| hugepages | 40    | Number of "huge pages" requested to allocate for the size defined just before (1 GB).
| hugepagesz | 2M    | Allows to set the size of "huge pages" to 2 MB.
| hugepages | 0    | Number of "huge pages" requested to allocate for the size defined just before (2 MB).
| default_hugepagesz| 1G | Default value for the "huge pages" size.
...
`phc2sys`. Covers <<ptp-scenario-oc,Scenario 1a>> through
...
<<ptp-scenario-bc-multi-nic-dpll,Scenario 3a>>.
...
The GNR-D Boundary Clock (<<ptp-scenario-bc-multi-nic-dpll,Scenario 3a>>) cannot be
...
`gnrd-ptp-dpll-setup.service`. Adjust the device names (`eno8703np0`, `ptp<N>`, `eno16495np0`, `eno16595np0`) and the

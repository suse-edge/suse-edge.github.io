...
In this quick start we're using the "qcow2" image `SUSE-Multi-Linux-Manager-Server.x86_64-{version-mlm}.0-Qcow-GM.qcow2` for {x86-64} that you can find at https://www.suse.com/download/suse-manager/ or in the SUSE Customer Center. This image will work as a virtual machine on hypervisors like KVM. Please always check for the newest version of the image and use it for new installations.
...
----
transactional-update register -p Multi-Linux-Manager-Server/{version-mlm}/x86_64 -r <REGCODE>
----


The product string depends on your hardware architecture! For example, if you are using SUSE Multi-Linux Manager on a 64-bit Arm system, the string is "Multi-Linux-Manager-Server/{version-mlm}/aarch64".
...
For a minimal lab setup, we can actually use the same virtual machine SUSE Multi-Linux Manager Server is running on. Please make sure that you have enough disk space in the virtual machine! This is not a recommended setup for production use. See <<eib-prerequisites>> for host operating systems we have tested Edge Image Builder with.
...
In this example, the root password would be "root". See <<configuring-os-users>> for creating password hashes for the secure password you want to use.

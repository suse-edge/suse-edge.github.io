...
Starting with {product} 3.7, the V2 Data Engine is recommended for new volumes.
Before enabling it, prepare the required block devices and review <<guides-suse-storage-v2-data-engine>>.
...
The following example creates a V2 `StorageClass`.
The V2 Data Engine and a schedulable block disk must already be configured as described in <<guides-suse-storage-v2-data-engine>>.
...
longhorn-v2-example  driver.longhorn.io   Delete          Immediate           true                   24s

localhost:~ # kubectl get pvc -n longhorn-system
NAME                STATUS   VOLUME                                     CAPACITY   ACCESS MODES   STORAGECLASS       AGE
longhorn-volv-pvc   Bound    pvc-f663a92e-ac32-49ae-b8e5-8a6cc29a7d1e   2Gi        RWO            longhorn-v2-example   54s
...
To use the V2 Data Engine, include the V2 values and ensure that the required raw block devices are prepared and registered as described in <<guides-suse-storage-v2-data-engine>>.
====

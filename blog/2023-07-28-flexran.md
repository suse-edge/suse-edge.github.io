---
slug: Flexran
title: Deploying FlexRan on Suse Edge ATIP
authors:
...
Intel® FlexRAN is a reference implementation for cloud enabled wireless access virtual network functions `VNFs`. It shows how to efficiently implement wireless access loads through flexible software architecture, Intel® Xeon® Scalable processors using Intel® Advanced Vector Extensions 512 `Intel® AVX 512` instruction set, and optimized network functions virtualization infrastructure `NFVi` with Intel-specific patches for the Data Plane Development Kit `DPDK`.
This article focuses on using [Rancher](https://www.rancher.com/) to perform cluster life cycle management, cloud-native microservices life cycle management, contained application resource management, and telemetry data monitoring with role-based access control, as well as the use of [SLE Micro RT](https://www.suse.com/products/realtime/) Operating System and [RKE2](https://docs.rke2.io/) kubernetes cluster to run the FlexRAN tests on top of that stack.
 
The architecture outlined in this document focuses on [ORAN Alliance 7-2x lower-level](https://www.o-ran.org/blog/20-new-o-ran-specifications-have-been-released-since-june-2020) split to front haul. We will use the FlexRAN library to build our test applications that will support front haul C-Plane and U-plane, which will be part of the O-DU components.

With this cloud-native setup, you can simplify the installation and configuration of Intel® FlexRAN on the `Suse Edge ATIP` (Adaptive Telco Infrastructure Platform) stack, enabling you to focus on site-specific vRAN customizations vRAN. Also, it demostrates that telecom containerised applications like DU/CU can be run on RKE2 cluster on top of the SLE Micro RT OS taking advantage of the accelerator cards. In addition, we show that we are applying the full power and potential of the accelerator card and CPU.
...
>
> For more infomration about Suse ATIP, see the official [Suse Edge ATIP](https://deploy-preview-68--frolicking-rolypoly-9c0c48.netlify.app/docs/product/atip/introduction)
...
- Rancher Management Cluster: This component will be used to manage the lifecycle of the RKE2 clusters deployed for the FlexRAN solution, as well as the monitoring platform based on Grafana and Prometheus. This cluster is based on a single-node RKE2 cluster on top of SLE Micro Operating System.
...
- Operating system: `SLE Micro RT 5.4` 
...
> The operating system installation steps have been omitted for the sake of brevity.
...
This section covers the installation of the edge cluster nodes in order to deploy the FlexRan tests. In our case, we will deploy two edge clusters, one for the FlexRan Timer Mode tests and another one for the FlexRan Xran Mode tests.
...
Modify the `/etc/default/grub` file to add the cpu cores to isolate. The next options have to be customized with your current hardware environment:
...
> If you want to install the RKE2 cluster from the Rancher UI you can follow the next [document](https://suse-edge.github.io/docs/product/atip/edge-site#edge-site-definition)
...
2. Create the `/etc/rancher/rke2/config.yaml` file with the next content to enable `Multus + Calico` CNI plugins:

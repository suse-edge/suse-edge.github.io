...
EOF_DC
        # link images
        echo "Linking images for $lang"
        images=( $(daps -d "${REPO_ROOT}/$DC_FILE" list-srcfiles --imgonly) )
        pushd "${REPO_ROOT}/${PO4A_DIR}/translated/$lang/images/" >/dev/null
        for img in "${images[@]}"; do
            ln -s "$(realpath -e --relative-to=. $img)"
        done
        popd >/dev/null
    done
    echo
